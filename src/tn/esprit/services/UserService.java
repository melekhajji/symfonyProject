...
public class UserService {
 ...
 public void ajoutUser(User user) {
            ...
            String Url ="http://localhost:8000/user/signup?id="+user.getId()+"&email="+user.getEmail()+"&roles="+user.getRoles()+"&password="+user.getPassword()+"&cin="+user.getCin()+"&username="+user.getUsername();
 ...
 public void updateUser(User user) {
     ...
     public void deleteUser(int id_user) {
        ConnectionRequest con = new ConnectionRequest();// création d'une nouvelle demande de connexion
     String Url ="http://localhost:8000/user/remove/" +id_user+"/";
     ...
     public String getPassswordByEmail(String email,Resources rs){
       ConnectionRequest con = new ConnectionRequest();// création d'une nouvelle demande de connexion
       String Url ="http://localhost:8000/user/getPasswordByEmail?email" +email+"/";
       con.setUrl(Url);
       con.addResponseListener((e) -> {
           try {
                Map<String, Object> password = jSONParser.parseJSON(new CharArrayReader(new String(con.getResponseData()).toCharArray()));
                String str = new String(con.getResponseData());//Récupération de la réponse du serveur
